...
fun <T> count (line : CharSequence, listener : TrackListener<T>) : T {
    line.foldIndexed (Track (0, 0, false)) { position, track, c ->
        when (track.garbage) {
            true  ->
                when (c) {
                    '>'  -> if (track.escaped) track.copy (escape = 0) else track.copy (escape = 0, garbage = false)
            ...
            false ->
                when (c) {
                    '<'   -> track.copy (escape = 0, garbage = true)
...
data class Track (val group : Int, val escape : Int, val garbage : Boolean) {
